...
| 6a | UI link loss while GUIDED | App link failsafe (`app.py:1155-1181`) | After `link_loss_timeout_s` with no client while underway → `anchor_hold` (hold position). | `tests/test_chaos.py::test_link_loss_guided_engages_anchor_hold`; also `tests/test_safety_power.py::test_link_failsafe_engages_after_timeout` |
