"""'Leif' -- the pure full-azimuth learned station-keeper (experimental)."""
import numpy as np
from vanchor.app import Runtime
from vanchor.core.config import load
from vanchor.core.models import ControlModeName, ManualSetpoint
from vanchor.controller.anchor_ml import AnchorLeifMode
...
def test_anchor_leif_command_engages_the_mode():
    rt = _rt()
    rt.controller.handle_command({"type": "anchor_leif", "radius_m": 6, "anchor": {"lat": 59.0, "lon": 13.0}})
    assert rt.state.mode is ControlModeName.ANCHOR_LEIF
    assert rt.state.anchor is not None
    assert rt.state.anchor_radius_m == 6


def test_leif_produces_bounded_manual_setpoint_and_updates_distance():
    rt = _rt()
    rt.controller.handle_command({"type": "anchor_leif", "radius_m": 5, "anchor": {"lat": 59.0, "lon": 13.0}})
    for _ in range(20):
...
def test_leif_is_pure_no_pid_base():
    """Leif's command is the net output directly (no pid_base term)."""
    m = AnchorLeifMode()
...
def test_leif_azimuth_rescales_to_boat_range():
    ...
    st = 1.0 * (AnchorLeifMode.TRAIN_AZIMUTH_DEG / 180.0)  # boat range 180 > trained 120
