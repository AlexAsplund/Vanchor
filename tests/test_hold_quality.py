"""Hold quality metric (#34): rolling RMS radial error + % time within
...
surfaced as ``hold_quality`` in telemetry."""

from __future__ import annotations

import pytest

from vanchor.controller.controller import HoldQuality
...
def test_anchor_hold_populates_quality_metric():
    ...
    h.run(seconds=90)
    st = h.state
    assert st.hold_holding_s > 0.0
    assert st.hold_pct_in_radius > 80.0    # calm water: holds easily
    assert 0.0 <= st.hold_rms_m < 5.0
...
def test_anchor_ml_populates_quality_metric_too():
    ...
    assert st.hold_holding_s > 0.0
    assert st.hold_pct_in_radius > 70.0
    assert st.hold_rms_m < 6.0
...
def test_metric_pauses_out_of_anchor_modes_and_resets_when_cleared():
    ...
    h.command({"type": "anchor_hold", "radius_m": 5.0})
    h.run(seconds=30)
    rms = h.state.hold_rms_m
    assert h.state.hold_holding_s > 0.0
    # Manual mode with the anchor still set: PAUSED (numbers frozen, the stale
    # distance_to_anchor_m must not pollute the metric).
    h.command({"type": "stop"})
    held = h.state.hold_holding_s
    h.run(seconds=10)
    assert h.state.hold_rms_m == rms
    assert h.state.hold_holding_s == held
    # Clearing the mark resets the metric to zero.
    h.state.anchor = None
    h.run(seconds=2)
    assert h.state.hold_rms_m == 0.0
    assert h.state.hold_pct_in_radius == 0.0
    assert h.state.hold_holding_s == 0.0
...
def test_new_mark_restarts_the_measurement():
    ...
    h.run(seconds=5)
    assert 0.0 < h.state.hold_holding_s < before
...
def test_quality_surfaced_in_telemetry_dict():
    ...
    h.command({"type": "anchor_hold", "radius_m": 5.0})
    h.run(seconds=20)
    q = h.state.to_dict()["hold_quality"]
